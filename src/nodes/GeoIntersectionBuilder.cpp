...
GeoIntersectionBuilder::GeoIntersectionBuilder()
    : rete::NodeBuilder("geo:intersection", rete::NodeBuilder::BuilderType::BUILTIN)
{
}

rete::Builtin::Ptr GeoIntersectionBuilder::buildBuiltin(rete::ArgumentList& args) const
{
    ...
    // create the node
    auto node = std::make_shared<GeoIntersectionNode>(std::move(geo1), std::move(geo2));
