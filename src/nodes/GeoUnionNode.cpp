...
rete::WME::Ptr GeoUnionNode::process(rete::Token::Ptr token)
{
    ...
    auto u = g1->geometry()->Union(g2->geometry());
    auto resultGeom = std::make_shared<GeosGeometry>(std::move(u));
    auto wme = std::make_shared<rete::TupleWME<GeosGeometryInterface::Ptr>>(resultGeom);
    return wme;
}

bool GeoUnionNode::operator==(const rete::BetaNode& other) const
{
    auto o = dynamic_cast<const GeoUnionNode*>(&other);
    if (!o) return false;
