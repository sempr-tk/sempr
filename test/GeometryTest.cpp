...
BOOST_AUTO_TEST_SUITE(GeometryTest)
    ...
    BOOST_AUTO_TEST_CASE(geom_union_all)
    {
        sempr::Core core;
        core.loadPlugins("src");
        core.loadPlugins("../src");
        ...
        core.addRules(
            "[EC<GeosGeomI>(?e ?g), GROUP BY (),"
            " geo:unionAll(?union ?g ), geo:area(?area ?union)"
            "-> (<union> <hasArea> ?area)]"
        );
        ...
        core.performInference();
        std::ofstream("geom_union_all.dot") << core.reasoner().net().toDot();
